...
async function main() {
  ...
  const jsVarKinds = await QuickJSLib.getJSVarKinds()
  const jsModes = await QuickJSLib.getJSModes()
  const pc2LineCodes = await QuickJSLib.getPC2LineCodes()
  const defineMethodFlags = await QuickJSLib.getDefineMethodFlags()
  ...
  // JSVarKind
  lines.push(`export enum JSVarKind {`)
  for (const [key, value] of Object.entries(jsVarKinds)) {
    lines.push(`  ${key} = ${value},`)
  }
  lines.push(`}`)
  lines.push(``)
  ...
  // DefineMethodFlag (from QuickJSBinding)
