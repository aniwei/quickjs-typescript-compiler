...
function extractDefineMethodFlags(): Record<string, number> {
  const headerPath = resolve(process.cwd(), 'third_party/QuickJS/src/core/function.h')
  const content = readFileSync(headerPath, 'utf-8')
  const regex = /#define\s+OP_DEFINE_METHOD_(METHOD|GETTER|SETTER|ENUMERABLE)\s+(\d+)/g
  ...
  const required = ['METHOD', 'GETTER', 'SETTER', 'ENUMERABLE']
  for (const key of required) {
    if (!(key in result)) {
      throw new Error(`Missing OP_DEFINE_METHOD_${key} in function.h`)
    }
  }
  return result
}
...
async function main() {
  ...
  const defineMethodFlags = extractDefineMethodFlags()
  ...
  // DefineMethodFlag (from function.h)
  lines.push(`export enum DefineMethodFlag {`)
  for (const [key, value] of Object.entries(defineMethodFlags)) {
    // Normalize names to match TS style
    const tsName = key.charAt(0) + key.slice(1).toLowerCase()
    lines.push(`  ${tsName} = ${value},`)
  }
  lines.push(`}`)
  lines.push(``)
