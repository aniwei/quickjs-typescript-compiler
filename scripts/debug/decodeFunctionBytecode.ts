...
function getTargetFunc(mod: any, cpoolIndex: number): FunctionBytecode {
  if (cpoolIndex < 0) {
    return mod.func as FunctionBytecode
  }
...
function main() {
  ...
  const tsFunc = getTargetFunc(tsMod, cpoolIndex)
  const wasmFunc = getTargetFunc(wasmMod, cpoolIndex)
