export type SafeJsonStringifyOptions = {
  pretty?: boolean
  /** Maximum string length to inline before truncating. Default: 200k chars. */
  maxStringLength?: number
  /** Maximum array length to inline before truncating. Default: 50k items. */
  ...
  return JSON.stringify(
    ...
    (_key, v) => {
      if (typeof v === 'string' && v.length > maxStringLength) {
        const head = v.slice(0, 5_000)
        const tail = v.slice(-5_000)
        return { __type: 'TruncatedString', length: v.length, head, tail }
      }

      if (Array.isArray(v) && v.length > maxArrayLength) {
        return { __type: 'TruncatedArray', length: v.length, head: v.slice(0, 100), tail: v.slice(-100) }
      }
