...
function buildWasm(opts: Options) {
  ...
  const wasmRoot = resolve(repoRoot, 'third_party/QuickJS/bytecode_wasm')
  const buildDir = resolve(wasmRoot, 'build-emscripten')
  const outJs = resolve(wasmRoot, 'output/quickjs_bytecode.js')
  const outWasm = resolve(wasmRoot, 'output/quickjs_bytecode.wasm')
