...
EMSCRIPTEN_BINDINGS(quickjs_bytecode_wasm) {
  ...
  emscripten::function("taro_js_bc_get_version", &taro_bc_get_version);
  emscripten::function("taro_js_bc_get_binary_version",
    emscripten::optional_override([](std::string input) {
      return taro_bc_get_binary_version(input);
    }));
  emscripten::function("taro_js_bc_get_binary_compatible",
    emscripten::optional_override([](std::string input) {
