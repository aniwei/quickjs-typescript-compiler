...
import { LabelResolver } from './compiler/LabelResolver2'
...
export class TypeScriptCompiler implements CompilerContext {
  ...
  compile(source: string, filename: string = 'input.ts'): Uint8Array {
    ...
    // enable strict mode, and the directive statements themselves do not produce bytecode.
    // (They are semantically no-ops at runtime, so QuickJS drops them, and the directive
    // string should not appear in the atom table.)
    ...
    //   The parser scans the directive prologue (leading string-literal statements),
    //   sets strict-mode flags ("use strict"), then rewinds (js_parse_seek_token) so
    //   the statements still get parsed/emitted normally in script/eval mode.
    ...
    for (let i = 0; i < sourceFile.statements.length; i++) {
      ...
      if (scriptPrologue && scriptPrologue.directiveStatementIndexes.has(i)) {
        // QuickJS still emits bytecode for directive-prologue string statements in
        // script/eval mode (they behave like ordinary expression statements that
        // update <ret>), while *also* using them to set js_mode.
  ...
  // QuickJS source reference: third_party/QuickJS/src/core/parser.c
  // - js_parse_directives() scans consecutive TOK_STRING statements and toggles strict mode
  //   when it sees "use strict"; then it rewinds to the original token position.
  ...
  private processFunctionDef(fd: FunctionDef): void {
    ...
    if (process.env.DEBUG_DUMP_LINE_NUM_PRE && fd.source && fd.source.includes('ev.buf[2] = 10')) {
      // Dump TempOpcode.OP_line_num stream before resolve_labels to diagnose pc2line mismatches.
      ...
      const fmtSnippet = (sourcePos: number) => {
        const start = Math.max(0, sourcePos - 24)
        const end = Math.min(fd.source!.length, sourcePos + 24)
        return JSON.stringify(fd.source!.slice(start, end))
      }

      console.log(`[DEBUG_DUMP_LINE_NUM_PRE] func bytecodeLen=${bcLen}`)
      let pos = 0
      let currentSourcePos = fd.sourcePos
      while (pos < bcLen) {
        const op = bcBuf[pos]
        ...
        if (op === TempOpcode.OP_line_num && pos + 5 <= bcLen) {
          const sp = readU32(bcBuf, pos + 1)
          currentSourcePos = sp
          const [line, col] = DebugInfoBuilder.getLineColCached(cache, fd.source, sp)
          console.log(
            `[DEBUG_DUMP_LINE_NUM_PRE] OP_line_num@${pos} sourcePos=${sp} line=${line} col=${col} snippet=${fmtSnippet(sp)}`
          )

          // Also show the next real opcode after the OP_line_num(s).
          let p = pos + opDef.size
          while (p < bcLen && bcBuf[p] === TempOpcode.OP_line_num) {
            const sp2 = readU32(bcBuf, p + 1)
            const [l2, c2] = DebugInfoBuilder.getLineColCached(cache, fd.source, sp2)
            console.log(
              `[DEBUG_DUMP_LINE_NUM_PRE]   (chained) OP_line_num@${p} sourcePos=${sp2} line=${l2} col=${c2} snippet=${fmtSnippet(sp2)}`
            )
            currentSourcePos = sp2
            p += 5
          }
          if (p < bcLen) {
            const op2 = bcBuf[p]
            const def2 = (op2 >= 182 && op2 <= 200 && TEMP_OPCODE_BY_CODE[op2]) ? TEMP_OPCODE_BY_CODE[op2] : OPCODE_BY_CODE[op2]
            const name2 = def2?.id ?? `op_${op2}`
            console.log(`[DEBUG_DUMP_LINE_NUM_PRE]   nextOp@${p} ${name2}`)
          }
        }

        // Spot-check push_i32 (often later shortened to push_i8) around the suspicious RHS.
        if (op === Opcode.OP_push_i32 && pos + 5 <= bcLen) {
          const val = (bcBuf[pos + 1] | (bcBuf[pos + 2] << 8) | (bcBuf[pos + 3] << 16) | (bcBuf[pos + 4] << 24)) | 0
          if (val === 10) {
            const [line, col] = DebugInfoBuilder.getLineColCached(cache, fd.source, currentSourcePos)
            console.log(
              `[DEBUG_DUMP_LINE_NUM_PRE]   saw push_i32 10 @${pos} currentSourcePos=${currentSourcePos} line=${line} col=${col}`
            )
          }
        }
    ...
    if (process.env.DEBUG_DUMP_PRE_RESOLVE_SMALL) {
      const bcBuf = fd.byteCode.buffer
      const bcLen = fd.byteCode.size
      if (bcLen > 0 && bcLen <= 220) {
        const hex = Array.from(bcBuf.slice(0, bcLen)).map(b => b.toString(16).padStart(2, '0')).join(' ')
        console.log(`[DEBUG_DUMP_PRE_RESOLVE_SMALL] bcLen=${bcLen} funcNameAtom=${fd.funcName} bytes=${hex}`)
      }
    }
  ...
  visit(node: ts.Node) {
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.WithStatement:
        this.statementVisitor.visitWithStatement(node as ts.WithStatement)
        break
      ...
      case ts.SyntaxKind.ClassExpression:
        this.classVisitor.visitClassExpression(node as ts.ClassExpression)
        break
