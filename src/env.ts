...
// Generated at: 2025-12-12T09:08:45.957Z
...
export enum Opcode {
  ...
  OP_is_undefined = 263,
  OP_is_null = 264,
  OP_typeof_is_undefined = 265,
  OP_typeof_is_function = 266,
  OP_get_field_ic = 267,
  OP_get_field2_ic = 268,
  OP_put_field_ic = 269,
  OP_debugger = 270,
}
...
// 创建从操作码数值到定义的映射
// 这用于在运行时通过数值查找操作码定义
function createOpcodeByCode(): Record<number, OpcodeDefinition & { name: string }> {
  const result: Record<number, OpcodeDefinition & { name: string }> = {}
  for (const [name, def] of Object.entries(OPCODE_DEFS)) {
    const code = (Opcode as any)[name] as number
    if (typeof code === 'number') {
      result[code] = { ...def, name }
    }
  }
  return result
}

export const OPCODE_BY_CODE: Record<number, OpcodeDefinition & { name: string }> = createOpcodeByCode()
