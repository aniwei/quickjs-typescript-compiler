...
type Edge = { start_pc: number; end_pc: number; target_pc: number };
...
function cleanDupSwapDrop(bc: number[]): number[] {
  ...
  for (let i = 0; i < bc.length; ) {
    const op = bc[i++]
    if (op === OP.push_i32 && i + 4 < bc.length && bc[i+4] === OP.drop) { 
      i += 5
      continue
    }
    ...
    out.push(op)
    if (hasI32Imm(op)) { 
      for (let k=0;k<4;k++,i++) out.push(bc[i])
    } else if (hasU16Imm(op)) { 
      out.push(bc[i++], bc[i++])
    } else if (hasI8Imm(op)) { 
      out.push(bc[i++])
...
function deadCodeElim(bc: number[]): number[] {
  ...
  for (let i = 0; i < bc.length; ) {
    ...
    out.push(op)
    if (op === OP.return || op === OP.throw || op === OP.goto || op === OP.goto8 || op === OP.goto16) {
      dead = true
    }
    if (hasI32Imm(op)) { 
      for (let k=0;k<4;k++,i++) {
