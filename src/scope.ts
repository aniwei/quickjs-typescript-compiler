...
export type Binding = { kind: 'param'|'local'; index: number; name: string; };
export type Scope = {
...
export class ScopeManager {
  ...
  constructor(rootNode: ts.SourceFile) {
    this.root = { func: rootNode, params: new Map(), locals: new Map(), parent: undefined };
  }

  enterFunction(fn: ts.FunctionLikeDeclaration, parent: Scope): Scope {
    const s: Scope = { func: fn, params: new Map(), locals: new Map(), parent };
  ...
  declareLocal(scope: Scope, name: string, idx: number) {
    if (!scope.locals.has(name)) scope.locals.set(name, idx);
  }
  ...
  resolve(scope: Scope, name: string): { level: number; bind?: Binding } {
    ...
    let level = 0;
    while (s) {
      if (s.params.has(name)) return { level, bind: { kind: 'param', index: s.params.get(name)!, name } };
      if (s.locals.has(name)) return { level, bind: { kind: 'local', index: s.locals.get(name)!, name } };
      s = s.parent; level++;
    }
    return { level };
  }
