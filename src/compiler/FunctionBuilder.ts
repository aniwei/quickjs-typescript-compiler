...
export class FunctionBuilder {
  ...
  static readonly JS_ATOM_TAG_INT = 0x80000000
  /**
   * 构建函数字节码
   * 
  ...
  private recomputeScopeLinkage(fd: FunctionDef): void {
    ...
    // QuickJS: function expression name (JS_VAR_FUNCTION_NAME) is created via add_var()
    // which zero-inits the JSVarDef, so scope_next defaults to 0.
    // For byte-for-byte parity we keep scope_next=0 for this special vardef.
    for (let idx = 0; idx < fd.varCount; idx++) {
      const vd = fd.vars[idx]
      if (vd.varKind === JSVarKindEnum.JS_VAR_FUNCTION_NAME) {
        vd.scopeNext = 0
      }
    }
...
export class BytecodeWriter {
  ...
  private shouldWriteWideString(str: string): boolean {
    // QuickJS serializes strings either as 8-bit bytes (wideFlag=0) or as UTF-16 code units (wideFlag=1).
    // A safe approximation of QuickJS's internal "wide" condition is: any UTF-16 code unit > 0xFF.
    // This also preserves unpaired surrogates (0xD800-0xDFFF) exactly.
  ...
  private writeJSString(str: string): void {
    const wide = this.shouldWriteWideString(str)
    const charLen = str.length

    // length << 1 | is_wide_char
    this.out.putULEB128((charLen << 1) | (wide ? 1 : 0))

    if (charLen === 0) return

    if (wide) {
      // Write UTF-16LE code units
      const bytes = new Uint8Array(charLen * 2)
      for (let i = 0; i < charLen; i++) {
        const u16 = str.charCodeAt(i)
        const off = i * 2
        bytes[off] = u16 & 0xff
        bytes[off + 1] = (u16 >>> 8) & 0xff
      }
      this.out.put(bytes)
      return
    }

    // Write 8-bit raw bytes (latin1-style: byte == code unit)
  ...
  private putAtom(atom: number): void {
    ...
    // QuickJS: if (__JS_AtomIsTaggedInt(atom)) { v = (atom << 1) | 1; }
    // See: third_party/QuickJS/src/core/bytecode.cpp:bc_put_atom
    // and third_party/QuickJS/src/core/string-utils.h:__JS_AtomIsTaggedInt
    if (((atom | 0) & (FunctionBuilder.JS_ATOM_TAG_INT | 0)) !== 0) {
      // __JS_AtomToUInt32(atom) = atom & ~JS_ATOM_TAG_INT
      const u32 = (atom & ~FunctionBuilder.JS_ATOM_TAG_INT) >>> 0
      v = (((u32 << 1) | 1) >>> 0)
      this.out.putULEB128(v)
      return
    }
  ...
  private finalizeWithAtoms(): Uint8Array {
    ...
    // 写入每个 atom 字符串 (QuickJS: JS_WriteString)
    for (const atom of this.idxToAtom) {
      ...
      const str = resolved || ''
      // Inline QuickJS-compatible string encoding into atomsOut
      const wide = this.shouldWriteWideString(str)
      const charLen = str.length
      atomsOut.putULEB128((charLen << 1) | (wide ? 1 : 0))
      if (charLen === 0) {
        continue
      }
      if (wide) {
