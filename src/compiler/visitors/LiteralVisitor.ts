...
export class LiteralVisitor extends VisitorContext {
  ...
  visitStringLiteral(node: ts.StringLiteral) {
    ...
    // QuickJS: TOK_STRING => emit_push_const(str, as_atom=1)
    // - Non-integer strings typically become atoms (OP_push_atom_value + ic)
    // - Numeric strings become tagged-int atoms and therefore fall back to OP_push_const
    this.compiler.emitPushConst(fd, text, true)
  }
