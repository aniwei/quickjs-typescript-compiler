...
export class BlockEnv {
  ...
  /**
   * Whether the current block has an active catch offset on the value stack.
   * Used to mirror QuickJS emit_return() behavior (OP_nip_catch) when returning
   * from within try/catch (even when there is no explicit finally block).
   */
  hasCatchOffset: boolean = false
...
export class FunctionDef {
  ...
  /**
   * When `hasParameterExpressions` is true, QuickJS may bind destructuring parameter
   * targets in the argument scope and then copy them into function-scope locals.
   * This captures the arg-scope local indices (source) for that copy.
   */
  pendingParamVarCopies?: Array<{ atom: number; srcVarIdx: number }>
  
  /** to reject directive in special cases */
  hasUseStrict: boolean = false

  /**
   * Atoms created from directive prologue string literals (e.g. "use strict", "use math").
   * QuickJS interns these strings as atoms even though the statements themselves do not
   * emit bytecode; we serialize them to keep the atom table byte-identical.
   */
