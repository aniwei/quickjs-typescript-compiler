...
export class Compiler {
  ...
  private static readonly JS_ATOM_TAG_INT = 0x80000000
  atoms: string[] = []
  ...
  getAtom(name: string): number | null {
    // QuickJS: numeric strings become tagged-int atoms: __JS_AtomFromUInt32(v) = v | JS_ATOM_TAG_INT
    // See: third_party/QuickJS/src/core/string-utils.h:is_num_string + __JS_AtomFromUInt32
    const taggedInt = Compiler.atomFromNumericString(name)
    if (taggedInt !== null) {
      return taggedInt
    }
  ...
  private static atomFromNumericString(name: string): number | null {
    // Mirror QuickJS numeric-atom behavior:
    // - First, is_num_string() recognizes 0..2^32-1 (string-utils.h)
    // - But JS_NewAtomUInt32 only returns a tagged-int atom when n <= JS_ATOM_MAX_INT (2^31-1)
    //   otherwise it creates a normal string atom (string-utils.c:JS_NewAtomUInt32).
    if (name.length === 0 || name.length > 10) {
      return null
    }
    if (name === '0') {
      return (Compiler.JS_ATOM_TAG_INT | 0) >>> 0
    }
    const c0 = name.charCodeAt(0)
    if (c0 < 0x31 /* '1' */ || c0 > 0x39 /* '9' */) {
      return null
    }
    // Fast reject non-digits
    ...
    return ((n >>> 0) | Compiler.JS_ATOM_TAG_INT) >>> 0
  }

  private static isTaggedIntAtom(atom: number): boolean {
    // Note: JS bitwise ops work on signed 32-bit; this check is still correct.
    return ((atom | 0) & (Compiler.JS_ATOM_TAG_INT | 0)) !== 0
  }
  ...
  getAtomString(atom: number): string | null {
    if (Compiler.isTaggedIntAtom(atom)) {
      // __JS_AtomToUInt32(atom) = atom & ~JS_ATOM_TAG_INT
      return String((atom & ~Compiler.JS_ATOM_TAG_INT) >>> 0)
    }
    // 检查是否为内置 atom
  ...
  addPc2LineInfo(fd: FunctionDef, pc: number, sourcePos: number) {
    ...
    if (this.sourceFile && process.env.DEBUG_PC2LINE) {
      ...
      console.log(
        `[COMP] addPc2LineInfo: pc=${pc} sourcePos=${sourcePos} line=${pos.line + 1} col=${pos.character + 1}`
      )
    }
  }
  ...
  isLiveCode(fd: FunctionDef): boolean {
    // Align with QuickJS js_is_live_code(): third_party/QuickJS/src/core/parser.c:1745-1763
    const pos = fd.lastOpcodePos
    if (pos < 0 || pos >= fd.byteCode.size) return true
  ...
  emitPushConst(fd: FunctionDef, val: any, asAtom: boolean = false): number {
    // QuickJS: emit_push_const(val, as_atom)
    // - If val is string and as_atom: JS_NewAtomStr(...)
    // - Only emit OP_push_atom_value if atom is not a tagged-int atom
    // See: third_party/QuickJS/src/core/parser.c:emit_push_const
    if (typeof val === 'string' && asAtom) {
      const atom = this.addAtom(val)
      if (atom !== JSAtom.JS_ATOM_null && !Compiler.isTaggedIntAtom(atom)) {
        this.emitOp(fd, Opcode.OP_push_atom_value)
        this.emitU32(fd, atom >>> 0)
        this.emitIc(fd, atom >>> 0)
        return 0
      }
    }
  ...
  /**
   * Pop scope without emitting OP_leave_scope.
   *
   * This is useful when the current block has no fallthrough path (e.g. a
   * single `break`/`continue` statement). In such cases, the scope is already
   * closed by `emitBreak()` via `closeScopes()`, and emitting an extra
   * OP_leave_scope in dead code can prevent later jump-to-next optimizations
   * from matching QuickJS output.
   */
  popScopeNoEmit(fd: FunctionDef): void {
    const scope = fd.scopeLevel
    fd.scopeLevel = fd.scopes[scope].parent
    fd.scopeFirst = this.getFirstLexicalVar(fd, fd.scopeLevel)
  }
   ...
   */
  emitReturnWithFinally(fd: FunctionDef, hasReturnValue: boolean, sourcePos?: number): void {
    // Mirror QuickJS: third_party/QuickJS/src/core/parser.c:6405-6510 (emit_return)
    // Also mirror QuickJS TOK_RETURN lowering: third_party/QuickJS/src/core/parser.c:6969-6991.
    // QuickJS calls emit_source_pos() once right before emit_return(), and emit_return()
    // may start by emitting synthetic stack ops (e.g. OP_undefined / OP_nip_catch).
    // To match pc2line parity, we must anchor the sourcePos to the *first* opcode
    // of this sequence (not necessarily the final OP_return).
    if (sourcePos != null && sourcePos !== -1) {
      this.emitSourcePos(fd, sourcePos)
      sourcePos = -1
    }
    let top = fd.topBreak
    ...
    while (top !== null) {
      // QuickJS triggers OP_nip_catch when an iterator or (possibly empty) finally handler is present.
      // Our compiler also marks try/catch blocks with hasCatchOffset to match the final optimized bytecode.
      if (top.hasIterator || top.labelFinally !== -1 || top.hasCatchOffset) {
        if (!hasReturnValue) {
          this.emitOp(fd, Opcode.OP_undefined)
          hasReturnValue = true
        }

        this.emitOp(fd, Opcode.OP_nip_catch)

        if (top.hasIterator) {
          // TODO: async-generator return iterator close sequence (not needed for current fixtures).
          this.emitOp(fd, Opcode.OP_rot3r)
          this.emitOp(fd, Opcode.OP_undefined) // dummy catch offset
          this.emitOp(fd, Opcode.OP_iterator_close)
        } else if (top.labelFinally !== -1) {
          this.emitGotoInt(fd, Opcode.OP_gosub, top.labelFinally)
        }
      }
    ...
    this.emitReturn(fd, hasReturnValue, sourcePos ?? -1)
  }
