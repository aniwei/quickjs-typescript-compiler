...
// if with && short-circuit
{
  ...
  const jf = code.filter(op => op === OpCode.OP_if_false || op === OpCode.OP_if_false8).length
...
// break/continue lowering presence
{
  ...
  const { code } = emitBytecode(ir)
  const jmpCount = code.filter(op => op === OpCode.OP_goto || op === OpCode.OP_goto8).length
...
// if-else with break/continue inside
{
  ...
  const { code } = emitBytecode(ir)
  const hasCondJmp = code.includes(OpCode.OP_if_false) || code.includes(OpCode.OP_if_false8)
  const hasJmp = code.includes(OpCode.OP_goto) || code.includes(OpCode.OP_goto8)
