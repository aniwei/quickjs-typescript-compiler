...
import { compileCallExpression } from './compiler/expressions/calls'
...
const DEBUG_TS_CLOSURE = process.env.DEBUG_TS_CLOSURE === '1'
const CLOSURE_DEBUG_OPCODES = new Set<number>([
  Opcode.OP_fclosure,
  Opcode.OP_fclosure8,
  ...
  Opcode.OP_put_var_ref_check,
  Opcode.OP_put_var_ref_check_init,
  Opcode.OP_get_var_ref,
  Opcode.OP_get_var_ref_check,
  Opcode.OP_get_var_ref0,
  Opcode.OP_get_var_ref1,
  Opcode.OP_get_var_ref2,
  Opcode.OP_get_var_ref3,
  Opcode.OP_tail_call,
...
export class Compiler {
  ...
  private describeAtom(atom: Atom): string {
    const name = this.atomTable.getAtomString(atom)
    return name ?? `<atom:${atom}>`
  }

  private formatNodeKind(node: ts.Node | null | undefined): string | null {
    return node ? ts.SyntaxKind[node.kind] ?? null : null
  }

  private getScopeDepth(scopeIndex: number): number {
    if (scopeIndex < 0) {
      return 0
    }
    let depth = 0
    let current = scopeIndex
    while (current >= 0) {
      depth += 1
      const scope = this.currentFunction.scopes[current]
      if (!scope) {
        break
      }
  ...
  private buildScopeDebugInfo(scopeIndex: number): Record<string, unknown> {
    if (scopeIndex < 0) {
      return { scopeIndex, depth: 0 }
    }
    const scope = this.currentFunction.scopes[scopeIndex]
    if (!scope) {
      return { scopeIndex, kind: 'unknown', depth: 0 }
    }
    return {
      scopeIndex,
      kind: scope.kind,
      parent: scope.parent,
      isVarScope: scope.isVarScope,
      varParent: scope.varParent,
      depth: this.getScopeDepth(scopeIndex),
    }
  }

  private debugClosureLog(event: string, info: Record<string, unknown>) {
    if (!DEBUG_TS_CLOSURE) {
      return
    }
    const funcAtom = this.currentFunction.bytecode.name
    const functionName = this.describeAtom(funcAtom)
    const funcKindName = FunctionKind[this.currentFunction.bytecode.funcKind] ?? this.currentFunction.bytecode.funcKind
    console.log('[TS][closure]', {
      event,
      functionName,
      funcKind: funcKindName,
  ...
  public compileChildFunction(
    ...
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('child.prepare', {
        childName: this.describeAtom(nameAtom),
        isArrowFunction,
        isAsync,
        isGenerator,
        funcKind: FunctionKind[funcKind] ?? funcKind,
        sourcePos,
      })
    }
  ...
  childFunction.bytecode.newTargetAllowed = !isArrowFunction
    ...
    if (DEBUG_TS_CLOSURE) {
      const initialScope = this.scopeManager.currentScope()
      this.debugClosureLog('child.enter', {
        childName: this.describeAtom(childFunction.bytecode.name),
        parentName: this.describeAtom(parentFunction.bytecode.name),
        initialScope: this.buildScopeDebugInfo(initialScope),
        scheduledSetups: scheduledSetups ? scheduledSetups.length : 0,
      })
    }
  ...
  this.currentFunction.bytecode.recordLineNumber(0, startLineColumn.line, startLineColumn.column, childFunction.sourcePos, 0)
    ...
    if (ts.isBlock(node.body)) {
      ...
      this.withSourceNode(expressionBody, () => {
        if (ts.isCallExpression(expressionBody)) {
          if (DEBUG_TS_CLOSURE) {
            this.debugClosureLog('arrow.tailCandidate', {
              nodeKind: this.formatNodeKind(expressionBody),
              argCount: expressionBody.arguments.length,
              calleeKind: this.formatNodeKind(expressionBody.expression),
            })
          }
          const usedTailCall = compileCallExpression(this, expressionBody, { tailPosition: true })
          if (DEBUG_TS_CLOSURE) {
            this.debugClosureLog('arrow.tailResult', {
              usedTailCall,
              nodeKind: this.formatNodeKind(expressionBody),
            })
          }
    ...
    this.popScope()
    if (DEBUG_TS_CLOSURE && this.pendingJumps.length > 0) {
      this.debugClosureLog('child.pendingJumps.beforeResolve', {
        pendingCount: this.pendingJumps.length,
      })
    }
    this.resolvePendingJumps()
    this.finalizeFunction(childFunction)
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('child.finalized', {
        childName: this.describeAtom(childFunction.bytecode.name),
        instructionCount: childFunction.bytecode.instructions.length,
        closureVarCount: childFunction.bytecode.closureVars.length,
        localVarCount: childFunction.vars.length,
        argCount: childFunction.args.length,
        stackSize: childFunction.bytecode.stackSize,
        explicitReturn: this.hasExplicitReturn,
      })
    }
    if (
    ...
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('child.restoreContext', {
        childName: this.describeAtom(childFunction.bytecode.name),
        restoredParent: this.describeAtom(this.currentFunction.bytecode.name),
      })
    }
  ...
  public emitLoadIdentifier(identifier: ts.Identifier) {
    ...
    const atom = this.atomTable.getAtomId(identifierName)
    if (this.argumentIndices.has(atom)) {
    ...
    if (this.closureVarIndices.has(atom)) {
      ...
      if (isLexicalClosure) {
        this.debugClosureLog('loadClosureVar', {
          identifier: identifierName,
          atom: this.describeAtom(atom),
          closureIndex: index,
          lexical: true,
          via: 'currentFunction',
          nodeKind: this.formatNodeKind(identifier),
        })
      ...
      if (shortOpcode !== undefined) {
        this.debugClosureLog('loadClosureVar', {
          ...
          via: 'currentFunction',
          nodeKind: this.formatNodeKind(identifier),
          opcode: Opcode[shortOpcode as Opcode] ?? shortOpcode,
        })
  ...
  private findCapturedVariableInParents(
    atom: Atom
  ): { functionDef: FunctionDef; variable: Var; varIndex: number; isArgument: boolean } | null {
    ...
    while (current) {
      for (let index = 0; index < current.vars.length; index += 1) {
        ...
        return { functionDef: current, variable, varIndex: index, isArgument: false }
      }
      for (let index = 0; index < current.args.length; index += 1) {
        const variable = current.args[index]
        if (variable.name !== atom) {
          continue
        }
        if (!variable.isCaptured) {
        ...
        return { functionDef: current, variable, varIndex: index, isArgument: true }
      }
  ...
  private ensureCapturedClosureVar(
    atom: Atom,
    capture: { functionDef: FunctionDef; variable: Var; varIndex: number; isArgument: boolean }
    ...
    if (this.closureVarIndices.has(atom)) {
      ...
      if (DEBUG_TS_CLOSURE) {
        this.debugClosureLog('ensureCapturedClosureVar.reuse', {
          atom: this.describeAtom(atom),
          closureIndex: existingIndex,
          isLexical,
          isArgument: closureVar?.isArgument ?? capture.isArgument,
          parentFunction: this.describeAtom(capture.functionDef.bytecode.name),
          parentVarIndex: capture.varIndex,
        })
      }
    ...
    const isArgument = parentClosureVar?.isArgument ?? capture.isArgument
    ...
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('ensureCapturedClosureVar.create', {
        atom: this.describeAtom(atom),
        closureIndex,
        isLexical: closureVar.isLexical,
        isConst: closureVar.isConst,
        isArgument: closureVar.isArgument,
        kind: closureVar.kind,
        parentFunction: this.describeAtom(capture.functionDef.bytecode.name),
        parentVarIndex: capture.varIndex,
        reusedParentClosureVar: Boolean(parentClosureVar),
      })
    }
  ...
  public emitFunctionClosure(constantIndex: number, node?: ts.Node | null) {
    const debugNode = node ?? null
    const previousSuppress = this.suppressDebugRecording
    ...
    try {
      ...
      this.suppressDebugRecording = previousSuppress
    }
  }
  ...
  private registerClosureVar(atom: Atom, varIndex: number, options: { isConst: boolean; isLet: boolean; capture?: boolean; kind?: VarKind }) {
    if (this.closureVarIndices.has(atom)) {
      if (DEBUG_TS_CLOSURE) {
        this.debugClosureLog('registerClosureVar.skip', {
          atom: this.describeAtom(atom),
          closureIndex: this.closureVarIndices.get(atom),
        })
      }
      return
    }
    ...
    this.closureVarIndices.set(atom, closureIndex)
    this.debugClosureLog('registerClosureVar', {
      atom: this.describeAtom(atom),
      varIndex,
      closureIndex,
      isConst: options.isConst,
  ...
  public emitStoreToLexical(atom: Atom, options: { suppressDebug?: boolean } = {}) {
    ...
    if (slot !== undefined) {
      const write = () => {
        this.debugClosureLog('storeLocalLexical', {
          atom: this.describeAtom(atom),
          slot,
          suppressDebug,
        })
    ...
    const writeClosure = () => {
      this.debugClosureLog('storeClosureVar', {
        atom: this.describeAtom(atom),
        closureIndex,
        suppressDebug,
      })
  ...
  private emitOpcode(
    ...
    const beforeDepth = this.stackDepth
    ...
    const afterDepth = this.stackDepth
    if (DEBUG_TS_CLOSURE && CLOSURE_DEBUG_OPCODES.has(opcode)) {
      this.debugClosureLog('emitOpcode', {
        opcode: Opcode[opcode] ?? opcode,
        operands: [...operands],
        instructionIndex,
        nodeKind: this.formatNodeKind(recordNode),
        beforeDepth,
        afterDepth,
        nPop,
        nPush,
  ...
  public pushScope(kind: ScopeKind = ScopeKind.Block) {
    ...
    const alreadyTracked = this.lexicalInitByScope.has(scopeIndex)
    if (!alreadyTracked) {
    ...
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('scope.push', {
        kind,
        scope: this.buildScopeDebugInfo(scopeIndex),
        lexicalInitTracked: this.lexicalInitByScope.has(scopeIndex),
        lexicalInitWasNew: !alreadyTracked,
      })
    }
  }
  ...
  public popScope() {
    ...
    if (popped !== undefined) {
      if (DEBUG_TS_CLOSURE) {
        const nextScope = this.scopeManager.currentScope()
        this.debugClosureLog('scope.pop', {
          popped: this.buildScopeDebugInfo(popped),
          next: this.buildScopeDebugInfo(nextScope),
          lexicalInitTracked: this.lexicalInitByScope.has(popped),
        })
      }
  ...
  private finalizeFunction(func: FunctionDef) {
    ...
    const displayName = funcName ?? this.describeAtom(func.bytecode.name)
    const initialClosureCount = func.bytecode.closureVars.length
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('finalize.begin', {
        targetName: displayName,
        instructionCount: func.bytecode.instructions.length,
        closureVarCount: initialClosureCount,
        varCount: func.vars.length,
        argCount: func.args.length,
      })
    }
    ...
    if (DEBUG_TS_CLOSURE) {
      const prunedCount = func.bytecode.closureVars.length
      if (prunedCount !== initialClosureCount) {
        this.debugClosureLog('finalize.afterPrune', {
          targetName: displayName,
          before: initialClosureCount,
          after: prunedCount,
        })
      }
    }
    ...
    if (DEBUG_TS_CLOSURE) {
      this.debugClosureLog('finalize.complete', {
        targetName: displayName,
        closureVarCount: func.bytecode.closureVars.length,
        lexicalVarCount: lexicalVars.length,
        argCount: func.args.length,
