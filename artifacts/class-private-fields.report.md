# 字节码分析报告

**输入文件**: __tests__/fixtures/class-private-fields.ts
**生成时间**: 2025-12-23T02:53:13.456Z

## 大小对比

- TypeScript编译器: 447 字节
- WASM编译器: 450 字节
- 差异: -3 字节 (-0.67%)

## 结构分析

### TypeScript编译器输出
```json
{
  "source": "TypeScript",
  "bcVersion": "0x5",
  "userAtomCount": 7,
  "atoms": [
    {
      "index": 0,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "Counter",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 12,
      "actualLength": 6,
      "isWideChar": 0,
      "string": "#count",
      "offset": 10
    },
    {
      "index": 2,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "c",
      "offset": 17
    },
    {
      "index": 3,
      "rawLength": 18,
      "actualLength": 9,
      "isWideChar": 0,
      "string": "increment",
      "offset": 19
    },
    {
      "index": 4,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 29
    },
    {
      "index": 5,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 37
    },
    {
      "index": 6,
      "rawLength": 84,
      "actualLength": 42,
      "isWideChar": 0,
      "string": "__tests__/fixtures/class-private-fields.js",
      "offset": 41
    }
  ],
  "functionHeader": {
    "offset": 84,
    "tag": "0xc",
    "remaining": 363
  }
}
```

### WASM编译器输出
```json
{
  "source": "WASM",
  "bcVersion": "0x5",
  "userAtomCount": 7,
  "atoms": [
    {
      "index": 0,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "Counter",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 12,
      "actualLength": 6,
      "isWideChar": 0,
      "string": "#count",
      "offset": 10
    },
    {
      "index": 2,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "c",
      "offset": 17
    },
    {
      "index": 3,
      "rawLength": 18,
      "actualLength": 9,
      "isWideChar": 0,
      "string": "increment",
      "offset": 19
    },
    {
      "index": 4,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 29
    },
    {
      "index": 5,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 37
    },
    {
      "index": 6,
      "rawLength": 84,
      "actualLength": 42,
      "isWideChar": 0,
      "string": "__tests__/fixtures/class-private-fields.js",
      "offset": 41
    }
  ],
  "functionHeader": {
    "offset": 84,
    "tag": "0xc",
    "remaining": 366
  }
}
```

## 字节级差异

共发现 132 个字节差异:

- 偏移量 0x121: TS=0x02 vs WASM=0x04
- 偏移量 0x122: TS=0x00 vs WASM=0x01
- 偏移量 0x124: TS=0x0f vs WASM=0x0b
- 偏移量 0x12a: TS=0x08 vs WASM=0xca
- 偏移量 0x12b: TS=0xcb vs WASM=0x03
- 偏移量 0x12c: TS=0xc7 vs WASM=0x02
- 偏移量 0x12d: TS=0x43 vs WASM=0x0d
- 偏移量 0x12e: TS=0xe5 vs WASM=0x08
- 偏移量 0x12f: TS=0x00 vs WASM=0xcb
- 偏移量 0x130: TS=0x00 vs WASM=0xc7
- 偏移量 0x131: TS=0x00 vs WASM=0x11
- 偏移量 0x132: TS=0x91 vs WASM=0xdf
- 偏移量 0x133: TS=0x44 vs WASM=0x45
- 偏移量 0x134: TS=0xe5 vs WASM=0x93
- 偏移量 0x135: TS=0x00 vs WASM=0x18
- 偏移量 0x136: TS=0x00 vs WASM=0xdf
- 偏移量 0x137: TS=0x00 vs WASM=0x46
- 偏移量 0x13b: TS=0x06 vs WASM=0x08
- 偏移量 0x140: TS=0x20 vs WASM=0x07
- 偏移量 0x141: TS=0x16 vs WASM=0x08
- ... (显示前20个差异，总共132个)

## 十六进制转储对比

### TypeScript
```
00000000: 05 07 0e 43 6f 75 6e 74 65 72 0c 23 63 6f 75 6e |...Counter.#coun|
00000010: 74 02 63 12 69 6e 63 72 65 6d 65 6e 74 0e 63 6f |t.c.increment.co|
00000020: 6e 73 6f 6c 65 06 6c 6f 67 54 5f 5f 74 65 73 74 |nsole.logT__test|
00000030: 73 5f 5f 2f 66 69 78 74 75 72 65 73 2f 63 6c 61 |s__/fixtures/cla|
00000040: 73 73 2d 70 72 69 76 61 74 65 2d 66 69 65 6c 64 |ss-private-field|
00000050: 73 2e 6a 73 0c 00 06 00 a4 01 00 04 00 03 00 04 |s.js............|
00000060: 86 01 04 a6 01 00 00 00 c8 03 02 00 30 ca 03 03 |............0...|
00000070: 02 75 f0 01 03 03 70 40 e4 00 00 00 80 40 e6 00 |.u....p@.....@..|
00000080: 00 00 80 3f e4 00 00 00 82 3f e6 00 00 00 80 63 |...?.....?.....c|
00000090: 01 00 06 63 03 00 63 02 00 c1 02 58 e4 00 00 00 |...c..c....X....|
000000a0: 00 05 e5 00 00 00 cd c2 00 56 e7 00 00 00 00 c2 |.........V......|
000000b0: 01 56 42 00 00 00 01 c2 03 52 ce 0e d0 6b 03 00 |.VB......R...k..|
000000c0: 6b 02 00 3b e4 00 00 00 39 e4 00 00 00 11 21 00 |k..;....9.....!.|
000000d0: 00 3b e6 00 00 00 39 e6 00 00 00 43 e7 00 00 00 |.;....9....C....|
000000e0: 24 00 00 cb 39 e8 00 00 00 43 e9 00 00 00 39 e6 |$...9....C....9.|
000000f0: 00 00 00 42 42 00 00 00 24 01 00 cf 28 d4 03 18 |...BB...$...(...|
00000100: 00 00 00 51 12 1c 20 0e 2b 29 1b 02 1b 14 17 15 |...Q.. .+)......|
00000110: 1b 0e 1b 0a 1b 02 1b 03 00 0c 42 07 01 00 00 01 |..........B.....|
00000120: 00 02 00 00 0f 01 10 00 01 00 08 cb c7 43 e5 00 |.............C..|
00000130: 00 00 91 44 e5 00 00 00 29 d4 03 06 02 04 0d 08 |...D....).......|
00000140: 20 16 00 0c 42 07 01 00 00 01 00 01 00 00 09 01 | ...B...........|
00000150: 10 00 01 00 08 cb c7 42 e5 00 00 00 28 d4 03 06 |.......B....(...|
00000160: 05 04 12 1e 1b 15 00 0c 40 05 01 00 00 01 00 02 |........@.......|
00000170: 01 00 0f 01 10 00 01 00 f0 01 03 0d 08 cb 2b 68 |..............+h|
00000180: 00 00 11 ec 06 c7 1b 24 00 00 29 d4 03 02 08 00 |.......$..).....|
00000190: 00 0c 48 05 01 00 00 02 00 03 01 00 0e 02 10 00 |..H.............|
000001a0: 01 00 ea 01 00 01 00 ca 03 02 5d 0c 04 cc 08 cb |..........].....|
000001b0: ee 01 c7 68 00 00 b7 47 29 d4 03 02 00 00 00    |...h...G)......|
```

### WASM
```
00000000: 05 07 0e 43 6f 75 6e 74 65 72 0c 23 63 6f 75 6e |...Counter.#coun|
00000010: 74 02 63 12 69 6e 63 72 65 6d 65 6e 74 0e 63 6f |t.c.increment.co|
00000020: 6e 73 6f 6c 65 06 6c 6f 67 54 5f 5f 74 65 73 74 |nsole.logT__test|
00000030: 73 5f 5f 2f 66 69 78 74 75 72 65 73 2f 63 6c 61 |s__/fixtures/cla|
00000040: 73 73 2d 70 72 69 76 61 74 65 2d 66 69 65 6c 64 |ss-private-field|
00000050: 73 2e 6a 73 0c 00 06 00 a4 01 00 04 00 03 00 04 |s.js............|
00000060: 86 01 04 a6 01 00 00 00 c8 03 02 00 30 ca 03 03 |............0...|
00000070: 02 75 f0 01 03 03 70 40 e4 00 00 00 80 40 e6 00 |.u....p@.....@..|
00000080: 00 00 80 3f e4 00 00 00 82 3f e6 00 00 00 80 63 |...?.....?.....c|
00000090: 01 00 06 63 03 00 63 02 00 c1 02 58 e4 00 00 00 |...c..c....X....|
000000a0: 00 05 e5 00 00 00 cd c2 00 56 e7 00 00 00 00 c2 |.........V......|
000000b0: 01 56 42 00 00 00 01 c2 03 52 ce 0e d0 6b 03 00 |.VB......R...k..|
000000c0: 6b 02 00 3b e4 00 00 00 39 e4 00 00 00 11 21 00 |k..;....9.....!.|
000000d0: 00 3b e6 00 00 00 39 e6 00 00 00 43 e7 00 00 00 |.;....9....C....|
000000e0: 24 00 00 cb 39 e8 00 00 00 43 e9 00 00 00 39 e6 |$...9....C....9.|
000000f0: 00 00 00 42 42 00 00 00 24 01 00 cf 28 d4 03 18 |...BB...$...(...|
00000100: 00 00 00 51 12 1c 20 0e 2b 29 1b 02 1b 14 17 15 |...Q.. .+)......|
00000110: 1b 0e 1b 0a 1b 02 1b 03 00 0c 42 07 01 00 00 01 |..........B.....|
00000120: 00 04 01 00 0b 01 10 00 01 00 ca 03 02 0d 08 cb |................|
00000130: c7 11 df 45 93 18 df 46 29 d4 03 08 02 04 0d 08 |...E...F).......|
00000140: 07 08 11 0e 00 0c 42 07 01 00 00 01 00 02 01 00 |......B.........|
00000150: 06 01 10 00 01 00 ca 03 02 0d 08 cb c7 df 45 28 |..............E(|
00000160: d4 03 06 05 04 12 1e 0c 15 00 0c 40 05 01 00 00 |...........@....|
00000170: 01 00 02 01 00 0f 01 10 00 01 00 f0 01 03 0d 08 |................|
00000180: cb 2b 68 00 00 11 ec 06 c7 1b 24 00 00 29 d4 03 |.+h.......$..)..|
00000190: 02 08 00 00 0c 48 05 01 00 00 02 00 03 01 00 0e |.....H..........|
000001a0: 02 10 00 01 00 ea 01 00 01 00 ca 03 02 5d 0c 04 |.............]..|
000001b0: cc 08 cb ee 01 c7 68 00 00 b7 47 29 d4 03 02 00 |......h...G)....|
000001c0: 00 00                                           |..|
```