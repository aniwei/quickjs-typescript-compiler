# 字节码分析报告

**输入文件**: /Users/aniwei/Desktop/workspaces/quickjs-typescript-compiler_second/__tests__/compiler/fixtures/postfix-unary.ts
**生成时间**: 2025-12-08T15:56:10.262Z

## 大小对比

- TypeScript编译器: 172 字节
- WASM编译器: 188 字节
- 差异: -16 字节 (-8.51%)

## 结构分析

### TypeScript编译器输出
```json
{
  "source": "TypeScript",
  "bcVersion": "0x5",
  "userAtomCount": 4,
  "atoms": [
    {
      "index": 0,
      "rawLength": 88,
      "actualLength": 44,
      "isWideChar": 0,
      "string": "__tests__/compiler/fixtures/postfix-unary.js",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "a",
      "offset": 47
    },
    {
      "index": 2,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 49
    },
    {
      "index": 3,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 57
    }
  ],
  "functionHeader": {
    "offset": 61,
    "tag": "0xd",
    "remaining": 111
  }
}
```

### WASM编译器输出
```json
{
  "source": "WASM",
  "bcVersion": "0x5",
  "userAtomCount": 4,
  "atoms": [
    {
      "index": 0,
      "rawLength": 88,
      "actualLength": 44,
      "isWideChar": 0,
      "string": "__tests__/compiler/fixtures/postfix-unary.js",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "a",
      "offset": 47
    },
    {
      "index": 2,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 49
    },
    {
      "index": 3,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 57
    }
  ],
  "functionHeader": {
    "offset": 61,
    "tag": "0xd",
    "remaining": 127
  }
}
```

## 字节级差异

共发现 75 个字节差异:

- 偏移量 0x51: TS=0x3b vs WASM=0x43
- 偏移量 0x67: TS=0xdf vs WASM=0x68
- 偏移量 0x68: TS=0x93 vs WASM=0x00
- 偏移量 0x69: TS=0xe3 vs WASM=0x00
- 偏移量 0x6a: TS=0x24 vs WASM=0x93
- 偏移量 0x6b: TS=0x01 vs WASM=0x69
- 偏移量 0x6d: TS=0x0e vs WASM=0x00
- 偏移量 0x6e: TS=0x39 vs WASM=0x24
- 偏移量 0x6f: TS=0xe6 vs WASM=0x01
- 偏移量 0x71: TS=0x00 vs WASM=0x0e
- 偏移量 0x72: TS=0x00 vs WASM=0x39
- 偏移量 0x73: TS=0x43 vs WASM=0xe6
- 偏移量 0x74: TS=0xe7 vs WASM=0x00
- 偏移量 0x77: TS=0x00 vs WASM=0x43
- 偏移量 0x78: TS=0xdf vs WASM=0xe7
- 偏移量 0x79: TS=0x92 vs WASM=0x00
- 偏移量 0x7a: TS=0xe3 vs WASM=0x00
- 偏移量 0x7b: TS=0x24 vs WASM=0x00
- 偏移量 0x7c: TS=0x01 vs WASM=0x68
- 偏移量 0x7e: TS=0x0e vs WASM=0x00
- ... (显示前20个差异，总共75个)

## 十六进制转储对比

### TypeScript
```
00000000: 05 04 58 5f 5f 74 65 73 74 73 5f 5f 2f 63 6f 6d |..X__tests__/com|
00000010: 70 69 6c 65 72 2f 66 69 78 74 75 72 65 73 2f 70 |piler/fixtures/p|
00000020: 6f 73 74 66 69 78 2d 75 6e 61 72 79 2e 6a 73 02 |ostfix-unary.js.|
00000030: 61 0e 63 6f 6e 73 6f 6c 65 06 6c 6f 67 0d c8 03 |a.console.log...|
00000040: 00 00 00 00 00 0c 20 06 01 a4 01 00 00 00 04 01 |...... .........|
00000050: 00 3b 00 ca 03 00 09 08 ec 02 29 b8 e3 39 e6 00 |.;........)..9..|
00000060: 00 00 43 e7 00 00 00 df 93 e3 24 01 00 0e 39 e6 |..C.......$...9.|
00000070: 00 00 00 43 e7 00 00 00 df 92 e3 24 01 00 0e 39 |...C.......$...9|
00000080: e6 00 00 00 43 e7 00 00 00 68 00 00 24 01 00 0e |....C....h..$...|
00000090: 06 2f c8 03 16 00 00 21 00 1b 07 2a 04 17 75 1b |./.....!...*..u.|
000000a0: 07 2a 04 17 75 1b 07 1b 05 11 7f 00             |.*..u.......|
```

### WASM
```
00000000: 05 04 58 5f 5f 74 65 73 74 73 5f 5f 2f 63 6f 6d |..X__tests__/com|
00000010: 70 69 6c 65 72 2f 66 69 78 74 75 72 65 73 2f 70 |piler/fixtures/p|
00000020: 6f 73 74 66 69 78 2d 75 6e 61 72 79 2e 6a 73 02 |ostfix-unary.js.|
00000030: 61 0e 63 6f 6e 73 6f 6c 65 06 6c 6f 67 0d c8 03 |a.console.log...|
00000040: 00 00 00 00 00 0c 20 06 01 a4 01 00 00 00 04 01 |...... .........|
00000050: 00 43 00 ca 03 00 09 08 ec 02 29 b8 e3 39 e6 00 |.C........)..9..|
00000060: 00 00 43 e7 00 00 00 68 00 00 93 69 00 00 24 01 |..C....h...i..$.|
00000070: 00 0e 39 e6 00 00 00 43 e7 00 00 00 68 00 00 92 |..9....C....h...|
00000080: 69 00 00 24 01 00 0e 39 e6 00 00 00 43 e7 00 00 |i..$...9....C...|
00000090: 00 68 00 00 24 01 00 0e 06 2f c8 03 1e 00 00 21 |.h..$..../.....!|
000000a0: 00 1b 0e 1b 0a 11 02 16 03 17 15 1b 0e 1b 0a 11 |................|
000000b0: 02 16 03 17 15 1b 0e 1b 0a 11 01 00             |............|
```