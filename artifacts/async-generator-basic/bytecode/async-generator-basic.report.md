# 字节码分析报告

**输入文件**: /Users/aniwei/Desktop/workspaces/quickjs-typescript-compiler/__tests__/fixtures/async-generator-basic.ts
**生成时间**: 2025-12-26T13:59:18.757Z

## 大小对比

- TypeScript编译器: 285 字节
- WASM编译器: 287 字节
- 差异: -2 字节 (-0.70%)

## 结构分析

### TypeScript编译器输出
```json
{
  "source": "TypeScript",
  "bcVersion": "0x5",
  "userAtomCount": 6,
  "atoms": [
    {
      "index": 0,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "gen",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 86,
      "actualLength": 43,
      "isWideChar": 0,
      "string": "__tests__/fixtures/async-generator-basic.js",
      "offset": 6
    },
    {
      "index": 2,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "sum",
      "offset": 50
    },
    {
      "index": 3,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "v",
      "offset": 54
    },
    {
      "index": 4,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 56
    },
    {
      "index": 5,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 64
    }
  ],
  "functionHeader": {
    "offset": 68,
    "tag": "0xc",
    "remaining": 217
  }
}
```

### WASM编译器输出
```json
{
  "source": "WASM",
  "bcVersion": "0x5",
  "userAtomCount": 6,
  "atoms": [
    {
      "index": 0,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "gen",
      "offset": 2
    },
    {
      "index": 1,
      "rawLength": 86,
      "actualLength": 43,
      "isWideChar": 0,
      "string": "__tests__/fixtures/async-generator-basic.js",
      "offset": 6
    },
    {
      "index": 2,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "sum",
      "offset": 50
    },
    {
      "index": 3,
      "rawLength": 2,
      "actualLength": 1,
      "isWideChar": 0,
      "string": "v",
      "offset": 54
    },
    {
      "index": 4,
      "rawLength": 14,
      "actualLength": 7,
      "isWideChar": 0,
      "string": "console",
      "offset": 56
    },
    {
      "index": 5,
      "rawLength": 6,
      "actualLength": 3,
      "isWideChar": 0,
      "string": "log",
      "offset": 64
    }
  ],
  "functionHeader": {
    "offset": 68,
    "tag": "0xc",
    "remaining": 219
  }
}
```

## 字节级差异

共发现 51 个字节差异:

- 偏移量 0xc3: TS=0x36 vs WASM=0x38
- 偏移量 0xdf: TS=0x0c vs WASM=0x0e
- 偏移量 0xe8: TS=0x65 vs WASM=0x11
- 偏移量 0xe9: TS=0x00 vs WASM=0x65
- 偏移量 0xeb: TS=0x83 vs WASM=0x00
- 偏移量 0xec: TS=0x8d vs WASM=0x0e
- 偏移量 0xed: TS=0x85 vs WASM=0x83
- 偏移量 0xee: TS=0xec vs WASM=0x8d
- 偏移量 0xef: TS=0xf1 vs WASM=0x85
- 偏移量 0xf0: TS=0x0e vs WASM=0xec
- 偏移量 0xf1: TS=0x86 vs WASM=0xef
- 偏移量 0xf2: TS=0x39 vs WASM=0x0e
- 偏移量 0xf3: TS=0xe8 vs WASM=0x86
- 偏移量 0xf4: TS=0x00 vs WASM=0x39
- 偏移量 0xf5: TS=0x00 vs WASM=0xe8
- 偏移量 0xf7: TS=0x43 vs WASM=0x00
- 偏移量 0xf8: TS=0xe9 vs WASM=0x00
- 偏移量 0xf9: TS=0x00 vs WASM=0x43
- 偏移量 0xfa: TS=0x00 vs WASM=0xe9
- 偏移量 0xfc: TS=0x64 vs WASM=0x00
- ... (显示前20个差异，总共51个)

## 十六进制转储对比

### TypeScript
```
00000000: 05 06 06 67 65 6e 56 5f 5f 74 65 73 74 73 5f 5f |...genV__tests__|
00000010: 2f 66 69 78 74 75 72 65 73 2f 61 73 79 6e 63 2d |/fixtures/async-|
00000020: 67 65 6e 65 72 61 74 6f 72 2d 62 61 73 69 63 2e |generator-basic.|
00000030: 6a 73 06 73 75 6d 02 76 0e 63 6f 6e 73 6f 6c 65 |js.sum.v.console|
00000040: 06 6c 6f 67 0c 00 06 00 a4 01 00 01 00 01 00 02 |.log............|
00000050: 13 01 a6 01 00 00 00 40 e4 00 00 00 40 c2 00 41 |.......@....@..A|
00000060: e4 00 00 00 00 c2 01 f0 cf 28 ca 03 0a 00 00 00 |.........(......|
00000070: 0e 0c 00 00 02 0a 04 00 0c 72 06 00 c8 03 00 00 |.........r......|
00000080: 00 03 00 00 24 00 89 b8 8d 8a ec 04 8d 8d 2f 0e |....$........./.|
00000090: 39 c7 00 00 00 43 82 00 00 00 b9 24 01 00 8d 8d |9....C.....$....|
000000a0: 8a ec 04 8d 8d 2f 0e 06 8d 2f ca 03 0a 01 00 08 |...../.../......|
000000b0: 08 30 18 1b 0e 20 10 00 0c 22 06 00 00 00 02 00 |.0... ..."......|
000000c0: 05 00 00 36 02 cc 03 01 00 20 ce 03 02 01 30 63 |...6..... ....0c|
000000d0: 00 00 b7 cb 63 01 00 39 e4 00 00 00 f0 80 ee 0c |....c..9........|
000000e0: cc 64 00 00 64 01 00 9f 65 00 00 83 8d 85 ec f1 |.d..d...e.......|
000000f0: 0e 86 39 e8 00 00 00 43 e9 00 00 00 64 00 00 24 |..9....C....d..$|
00000100: 01 00 0e 06 2f ca 03 14 06 01 2c 32 1b 06 1c 29 |..../.....,2...)|
00000110: 11 0e 11 05 3a 0f 1b 0e 1b 0a 11 01 00          |....:........|
```

### WASM
```
00000000: 05 06 06 67 65 6e 56 5f 5f 74 65 73 74 73 5f 5f |...genV__tests__|
00000010: 2f 66 69 78 74 75 72 65 73 2f 61 73 79 6e 63 2d |/fixtures/async-|
00000020: 67 65 6e 65 72 61 74 6f 72 2d 62 61 73 69 63 2e |generator-basic.|
00000030: 6a 73 06 73 75 6d 02 76 0e 63 6f 6e 73 6f 6c 65 |js.sum.v.console|
00000040: 06 6c 6f 67 0c 00 06 00 a4 01 00 01 00 01 00 02 |.log............|
00000050: 13 01 a6 01 00 00 00 40 e4 00 00 00 40 c2 00 41 |.......@....@..A|
00000060: e4 00 00 00 00 c2 01 f0 cf 28 ca 03 0a 00 00 00 |.........(......|
00000070: 0e 0c 00 00 02 0a 04 00 0c 72 06 00 c8 03 00 00 |.........r......|
00000080: 00 03 00 00 24 00 89 b8 8d 8a ec 04 8d 8d 2f 0e |....$........./.|
00000090: 39 c7 00 00 00 43 82 00 00 00 b9 24 01 00 8d 8d |9....C.....$....|
000000a0: 8a ec 04 8d 8d 2f 0e 06 8d 2f ca 03 0a 01 00 08 |...../.../......|
000000b0: 08 30 18 1b 0e 20 10 00 0c 22 06 00 00 00 02 00 |.0... ..."......|
000000c0: 05 00 00 38 02 cc 03 01 00 20 ce 03 02 01 30 63 |...8..... ....0c|
000000d0: 00 00 b7 cb 63 01 00 39 e4 00 00 00 f0 80 ee 0e |....c..9........|
000000e0: cc 64 00 00 64 01 00 9f 11 65 00 00 0e 83 8d 85 |.d..d....e......|
000000f0: ec ef 0e 86 39 e8 00 00 00 43 e9 00 00 00 64 00 |....9....C....d.|
00000100: 00 24 01 00 0e 06 2f ca 03 14 06 01 2c 32 1b 06 |.$..../.....,2..|
00000110: 1c 29 11 0e 11 05 44 0f 1b 0e 1b 0a 11 01 00    |.)....D........|
```